...
export function isR2ChecksumError(error: unknown): boolean {
  ...
  return isR2Error(
    error,
    (msg) => msg.includes("sha-256") || msg.includes("10037"),
  );
}
...
export function isR2MultipartDoesNotExistError(error: unknown): boolean {
  ...
  return isR2Error(
    error,
    (msg) =>
      msg.includes("multipart upload does not exist") || msg.includes("10024"),
  );
}
...
export function isR2RangedReadHeaderError(error: unknown): boolean {
  ...
  return isR2Error(
    error,
    (msg) =>
      msg.includes("requested range is not satisfiable") ||
      msg.includes("10039"),
  );
}
...
export class RetryBucket {
  ...
  async head(
    ...parameters: Parameters<R2Bucket["head"]>
  ): ReturnType<R2Bucket["head"]> {
    return retry(() => this.bucket.head(...parameters), {
      params: this.params,
    });
  }

  async get(
    ...parameters: Parameters<R2Bucket["get"]>
  ...
  async delete(
    ...parameters: Parameters<R2Bucket["delete"]>
  ): ReturnType<R2Bucket["delete"]> {
    return retry(() => this.bucket.delete(...parameters), {
      params: this.params,
    });
  }
  ...
  async put(
    ...
    return retry(() => this.bucket.put(key, value, { sha256: checksum }), {
      params: this.params,
  ...
  async createMultipartUpload(
    ...parameters: Parameters<R2Bucket["createMultipartUpload"]>
  ): Promise<RetryMultipartUpload> {
    return new RetryMultipartUpload(
      await this.bucket.createMultipartUpload(...parameters),
      this.params,
    );
  }

  resumeMultipartUpload(
    ...parameters: Parameters<R2Bucket["resumeMultipartUpload"]>
  ): RetryMultipartUpload {
    return new RetryMultipartUpload(
      this.bucket.resumeMultipartUpload(...parameters),
      this.params,
    );
  }
}
...
export class RetryMultipartUpload {
  ...
  async uploadPart(
    ...
    return retry(
      () => this.r2MultipartUpload.uploadPart(partNumber, value),
      this.retryOptions(),
    );
  }
  ...
  async complete(
    ...parameters: Parameters<R2MultipartUpload["complete"]>
  ): ReturnType<R2MultipartUpload["complete"]> {
    return retry(
      () => this.r2MultipartUpload.complete(...parameters),
      this.retryOptions(),
    );
  }
